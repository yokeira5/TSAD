...
class FAG_double(object):
    ...
    def __call__(self, img, img2, window_size):
        ...
        if self.is_findobj:
            ...
            to_location_w = int(random.uniform(w_start, w_end - window_size - cut_size))
        ...
        aug2 = img2.copy()
        aug2.paste(patch, (to_location_w + window_size, to_location_h), mask=mask)

        # noise
        to_pil = ToPILImage()
        noise = to_pil(torch.rand((channels, self.load_size, self.load_size)) * 255.)
        # noise.paste(patch, (to_location_w, to_location_h), mask=mask)
        ...
        msk.paste(ones_patch, (to_location_w + window_size, to_location_h), mask=mask)
        msk = msk.resize((self.load_size, self.load_size), Image.NEAREST)
        img = img.resize((self.load_size, self.load_size), Image.LANCZOS)
        aug = aug.resize((self.load_size, self.load_size), Image.LANCZOS)
        aug2 = aug2.resize((self.load_size, self.load_size), Image.LANCZOS)
        # noise = noise.resize((self.load_size, self.load_size), Image.LANCZOS)
        ...
        if np.random.random() > 0.5:
            ...
            aug2 = self.AddGaussianNoise(aug2, amplitude=self.noise_amp, mask=msk)
        ...
        #
        # img_gray_array = local_binary_pattern(np.array(aug), n_points, radius, method='uniform')
        #
        # aug = Image.fromarray(img_gray_array, 'L')
        ...
        # noise4 = torch.cat((noise,msk), dim=0)
        # single_noise = transforms.ToTensor()(single_noise)

        # return img, aug, msk, noise
        return img, aug, aug2, msk, noise
