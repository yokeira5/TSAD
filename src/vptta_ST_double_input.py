...
class VPTTA:
    def __init__(self, config):
        ...
        target_test_dataset = ST_double(rf'{config.dataset_root}', target_list = config.Target_Dataset, is_train=False, numbers=100)
    ...
    def run(self):
        ...
        for batch, data in tqdm(enumerate(self.target_test_loader), total=len(self.target_test_loader), desc="Processing Batches"):
            ...
            for tr_iter in range(self.iters):
                prompt_x1, _ = self.prompt(x[:, :3,:,:])
                prompt_x2, _ = self.prompt(x[:,3:,:,:])

                prompt_x = torch.cat((prompt_x1.detach(), prompt_x2.detach()), dim=1)
            ...
            with torch.no_grad():
                ...
                prompt_x1, _ = self.prompt(x[:, :3,:,:])
                prompt_x2, low_freq = self.prompt(x[:,3:,:,:])

                prompt_x = torch.cat((prompt_x1.detach(), prompt_x2.detach()), dim=1)

                seg_output, fea, head_input = self.model(x)
                seg_output1, fea, head_input = self.model(prompt_x)
